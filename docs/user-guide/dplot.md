...
For high-resolution simulations (e.g., 15000x3000 grids), dplot automatically downsamples data toward roughly 1920x1080 pixels before rendering. It keeps every nth point using a single stride for both axes, so the aspect ratio is preserved and each dimension lands near, but not exactly at, the target (the example below reduces to 2143x429). This keeps plotting fast. Downsampling is applied both in sequential and parallel modes.
